...
flowchart LR
    %% --- Raw input ---
    WELL[(Oil Well)] -->|Crude Oil 120/min| FT{{Fractionating Tower}}

    %% --- Primary fractions, top of tower to bottom ---
    FT --> RG[Refinery Gas<br/>120/min]
    FT --> GAS[Gasoline<br/>80/min]
    FT --> KER[Kerosene<br/>60/min]
    FT --> DSL[Diesel<br/>45/min]
    FT --> RES[Heavy Residue<br/>45/min]

    %% --- Downstream chains ---
    RG --> PREF{{Plastic Refinery}} --> PLAS[Plastic 40/min]
    GAS --> GGEN{{Gasoline Generator}} --> PWR1[Power ~150 MW]
    KER --> JET{{Jet Workshop}} --> JFUEL[Jet Fuel 30/min]
    DSL --> DGEN{{Diesel Generator}} --> PWR2[Power ~250 MW]
    RES --> COKE{{Coker Unit}}
    COKE --> BIT[Bitumen 20/min]
    COKE --> LUBE[Lubricant 10/min]
...
```mermaid
flowchart TD
    RES[Heavy Residue 30/min] --> COKE{{Coker Unit}}
    COKE -->|66%| BIT[Bitumen 20/min]
    COKE -->|33%| LUBE[Lubricant 10/min]
